...
export class SubscriptionsComponent implements OnInit, OnDestroy {
  ...
  onSubscriptionClick(subscription: SubscriptionModel): void {
    ...
    this.isPayMode = false;
    this.displaySubscriptionDialog = true;
    // TODO: Open subscription form dialog when implemented
    console.log('Subscription clicked for edit:', subscription);
  }

  onPayClick(subscription: SubscriptionModel): void {
    // Open subscription form dialog in pay mode
    this.selectedSubscriptionId = subscription.id;
    this.isPayMode = true;
    this.displaySubscriptionDialog = true;
    // TODO: Open subscription form dialog when implemented with pay action
    console.log('Subscription clicked for pay:', subscription);
  }

  onDeleteClick(subscription: SubscriptionModel): void {
    // TODO: Show delete confirmation dialog
    const confirmed = confirm(
      this.translateService.instant('subscriptions.confirmDelete', { name: subscription.name })
    );
    if (confirmed) {
      this.subscriptionState.deleteSubscription(subscription.id)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.toastService.showSuccess(
              'subscriptions.deleteSuccess',
              'subscriptions.deleteSuccessMessage',
              { name: subscription.name }
            );
          },
          error: () => {
            this.toastService.showError(
              'subscriptions.deleteError',
              'subscriptions.deleteErrorMessage'
            );
          }
        });
    }
