  ...
  template: `
    <div class="container mx-auto px-4 py-6">
          ...
          *ngFor="let wallet of wallets$ | async" 
          ...
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              <div class="text-2xl">{{ wallet.iconType === 'EMOJI' ? wallet.iconValue : '🏦' }}</div>
              <div>
                <h3 class="font-semibold text-gray-900 dark:text-white">{{ wallet.name }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ getWalletTypeLabel(wallet.type) }}</p>
              </div>
            </div>
      ...
      <div *ngIf="!(loading$ | async) && !(wallets$ | async)?.length" class="text-center py-12">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-4">Get started by creating your first wallet to track your finances.</p>
        <button 
          (click)="openCreateModal()"
